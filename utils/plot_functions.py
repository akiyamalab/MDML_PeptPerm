...
font_path = "/mnt/fs/sugita/.local/share/fonts/Times New Roman.ttf"
...
COLOR_DICT = {
    ...
    'testset': ['#4C72B0', '#0FB2F2', '#77CFF2', '#BDE3F2', '#F29422'],
    'vintage_1': ['#3F3E3C', '#F2B53B', '#F4DFC0', '#F18274', '#D83F37'],
    'rainbow_1': ['#A8E6CF', '#DCEDC1', '#FFD3B6', '#FFAAA5', '#FF8B94'],
    'seaborn': ['#4B73B0', '#DD8553', '#55A868', '#C54F53'],
    'noble': ['#010221', '#0A7373', '#B7BF99', '#EDAA25', '#C43302'],
    'bar': ['#67151B'],
}
...
def plot_hist(data_list, label_list, x_name, y_name, fig_name,
    ...
    if x_ticks:
        if rotate_x_ticks:
            plt.xticks(x_ticks, rotation=rotate_x_ticks, fontweight=fontweight, fontsize=12)
        else:
    ...
    if text_data:
        ...
        for i in range(len(hist)):
            x = (edge[i]+edge[i+1])/2
            y = hist[i]
            plt.text(x, y+hist.max()/50, y, size=13, c='black', ha='center')
...
def plot_scatter(x_list, y_list, label_list, x_name, y_name, fig_name='',
                 ...
                 legend=True, legend_loc='upper left', legend_outside=False, legend_prop=None,
    ...
    if diagonal:
        if x_ticks:
            ...
            plt.plot(range(-9, -2), range(-9, -2), color='dimgrey', linestyle="dashed", alpha=0.8)
    ...
    if legend:
        if legend_outside:
            plt.legend(bbox_to_anchor=(1.05, 1), loc=legend_loc, borderaxespad=0, frameon=True, fontsize=legend_prop)
        else:
            plt.legend(frameon=True, loc=legend_loc, prop=legend_prop)
...
def plot_line(data_list, label_list, x_name, y_name,
    ...
    for i in range(len(data_list)):
        if yerr:
            plt.errorbar([_ for _ in range(len(data_list[i]))], data_list[i], yerr=yerr[i], label='',
                         color=color_list[i], marker=marker_list[i], markersize=10, linestyle=linestyle_list[i],
                         alpha=yerr_style.get('alpha', 1.0),
                         elinewidth=yerr_style.get('elinewidth', 1.0),
                         ecolor=yerr_style.get('ecolor', color_list[i]),
                         capsize=yerr_style.get('capsize', 3),
                         capthick=yerr_style.get('capthick', 1))
    ...
    if height_list_2:
        ...
        for name, height in zip(name_list, height_list):
            if yerr:
                plt.bar(name, height, align='center', width=0.6, label=label_list[0], color=color_list[0], \
                        yerr=yerr, capsize=3, ecolor='#262626')
            else:
                plt.bar(name, height, align='center', width=0.6, label=label_list[0], color=color_list[0])
        ...
        if text:
            for i in range(len(height)):
                ...
                y = height[i]
                err = yerr[i] if yerr else 0
                # IMPORTANT
                plt.text(x, y+err+height.max()/50, y, size=14, c='black', ha='center', fontweight='normal')
...
def plot_barh(name_list, width_list, label_list, color_list,
    ...
    if y_ticks: plt.yticks(y_ticks[0], y_ticks[1], fontweight=fontweight)
    else: plt.yticks(fontweight=fontweight)
    if x_lim: plt.xlim(x_lim[0], x_lim[1])
    if y_lim: plt.ylim(y_lim[0], y_lim[1])
    ...
    for name, width, label, color in zip(name_list, width_list, label_list, color_list):
        if xerr:
            plt.barh(name, width, align='center', height=0.6, label=label, color=color, \
                     xerr=xerr, capsize=3, ecolor='#262626')
        else:
            plt.barh(name, width, align='center', height=0.6, label=label, color=color)

        if text:
            for i in range(len(width)):
                y = list(range(len(width)))[i]
                x = width[i]
                err = xerr[i] if xerr else 0
                # IMPORTANT
                plt.text(x+err+width.max()/50, y, x, size=14, c='black', ha='center', fontweight='normal')

    if legend:
        handles, labels = plt.gca().get_legend_handles_labels()
        handles = handles[::-1]  # 反转图例句柄的顺序
        labels = labels[::-1]
        if legend_outside:
            plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0, frameon=True, handles=handles, labels=labels)
        else:
            plt.legend(frameon=True, handles=handles, labels=labels)
...
def plot_heatmap(data,
                # 'Blues' 'Reds' 'Greens
                 cmap='Blues', annot=False, cbar=False, annot_kws=None,
                 vmin=None, vmax=None,
    ...
    res = sns.heatmap(data, annot=annot, cmap=cmap, square=True, cbar=cbar, annot_kws=annot_kws)
...
def plot_boxplot(plot_nums, data, labels,
                 highlight_values=None,
                 row_nums=1,
                 title=None, x_name=None, y_name=None, sub_title=None,
                 fontweight='normal', wspace=0.4,
                 y_ticks=None, y_tick_size=14,
                 grid=True,
                 ):

    fig = plt.figure(figsize=(12, 4*row_nums))

    # 余白を設定
    plt.subplots_adjust(wspace=wspace)

    if title: fig.suptitle(title, y=1.02, fontweight=fontweight)
    if x_name: fig.supxlabel(x_name, fontweight=fontweight)
    if y_name: fig.supylabel(y_name, fontweight=fontweight)

    for i in range(plot_nums):
        plt.subplot(row_nums, plot_nums//row_nums, i+1)

        if sub_title: plt.title(sub_title[i], fontweight=fontweight)
        # box
        plt.boxplot([data[i]], labels=[labels[i]],
                    # patch_artist=True, # 塗りつぶし
                    # #55A868
                    medianprops=dict(color='#0FB2F2', linewidth=1.5),  # 中央値の線の設定
                    # showmeans=True, meanline=True,
                    zorder=1,
                    )

        # highlight value
        if highlight_values: plt.scatter([1], [highlight_values[i]], color='#C43302', marker='*', s=80, zorder=2)

        plt.xticks(fontsize=14, fontweight=fontweight)
        if y_ticks:
            plt.yticks(y_ticks[i], fontsize=y_tick_size, fontweight=fontweight)
